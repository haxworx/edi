...
static void
_tab_move(const char *dest_path, const char *src_path)
{
   Edi_Mainview_Panel *panel, *p1, *p2;
   Edi_Mainview_Item *item, *it1, *it2;
   Eina_List *l, *src_list, *dest_list;

   it1 = it2 = NULL;

   if (!strcmp(dest_path, src_path)) return;
   ...
   for (int i = 0; i < count; i++)
     {
        ...
        EINA_LIST_FOREACH(panel->items, l, item)
          {
             if (!strcmp(item->path, dest_path))
               {
                  it1 = item;
                  p1 = panel;
               }
             else if (!strcmp(item->path, src_path))
               {
                  it2 = item;
                  p2 = panel;
               }
   ...
   panel = p1;
   src_list = eina_list_data_find_list(panel->items, it2);
   dest_list = eina_list_data_find_list(panel->items, it1);
   if (!src_list || !dest_list) return;
   if (src_list->next == dest_list) return;

   panel->items = eina_list_remove_list(panel->items, src_list);
   dest_list = eina_list_data_find_list(panel->items, it1);
   panel->items = eina_list_prepend_relative_list(panel->items, it2, dest_list);
   ...
   edi_mainview_panel_item_select_path(panel, src_path);
}

static void
_tab_move_end(const char *src_path)
{
   Edi_Mainview_Panel *panel;
   Edi_Mainview_Item *item, *src_item = NULL;
   Eina_List *l, *src_list;
   int count = edi_mainview_panel_count();
   ...
   src_list = eina_list_data_find_list(panel->items, src_item);
   if (!src_list || !src_list->next) return;

   panel->items = eina_list_remove_list(panel->items, src_list);
   panel->items = eina_list_append(panel->items, src_item);
...
static void
_tab_move_by_cursor(const char *src_path, Evas_Coord cursor_x, Evas_Coord cursor_y)
{
   Edi_Mainview_Panel *panel;
   Edi_Mainview_Item *item, *src_item = NULL, *dest_item = NULL;
   Eina_List *l;
   ...
   evas_object_geometry_get(panel->tabs, &tx, &ty, &tw, &th);
   if (cursor_y < ty - 36 || cursor_y > ty + th + 36) return;

   EINA_LIST_FOREACH(panel->items, l, item)
     {
        Evas_Coord x, w;

        if (item == src_item) continue;

        evas_object_geometry_get(item->tab->button, &x, NULL, &w, NULL);
        if (cursor_x < x + (w / 2))
          {
             dest_item = item;
             break;
   ...
   if (dest_item)
     _tab_move(dest_item->path, src_path);
   else
     _tab_move_end(src_path);
}
                     ...
                     void *event_info)
{
   ...
   evas_object_move(tab->button_drag, ev->cur.canvas.x - tab->drag_x_offset,
                    ev->cur.canvas.y - tab->drag_y_offset);
   evas_object_show(tab->button_drag);
   _tab_move_by_cursor(_tab_move_src_path, ev->cur.canvas.x, ev->cur.canvas.y);
}
                  ...
                  void *event_info EINA_UNUSED)
{
   ...
   evas_object_event_callback_del(edi_main_win_get(), EVAS_CALLBACK_MOUSE_MOVE, _tab_move_display_cb);
                   ...
                   void *event_info)
{
   ...
   tab->drag_x_offset = ev->canvas.x - x;
   tab->drag_y_offset = ev->canvas.y - y;
   ...
   evas_object_event_callback_add(edi_main_win_get(), EVAS_CALLBACK_MOUSE_MOVE, _tab_move_display_cb, tab);
                 ...
                 void *event_info EINA_UNUSED)
{
   ...
   _tab_move(tab->path, _tab_move_src_path);
}
...
static Eina_Bool
_edi_mainview_panel_drop_path_open(Edi_Mainview_Panel *panel, const char *path)
{
   Edi_Path_Options *options;

   if (!path || !path[0] || !ecore_file_exists(path) || ecore_file_is_dir(path))
     return EINA_FALSE;
...
static Eina_Bool
_edi_mainview_panel_drop_open_list(Edi_Mainview_Panel *panel, const char *text)
{
   Eina_Bool opened = EINA_FALSE;
   char *line, *list, *saveptr = NULL;
   ...
   while (line)
     {
        ...
        if (uri)
          {
             if ((!uri->protocol || !strcmp(uri->protocol, "file")) &&
                 _edi_mainview_panel_drop_path_open(panel, uri->path))
               opened = EINA_TRUE;
        ...
        if (_edi_mainview_panel_drop_path_open(panel, line))
          opened = EINA_TRUE;
next:
        line = strtok_r(NULL, "\r\n", &saveptr);
     }
...
static Eina_Bool
_edi_mainview_panel_external_drop_cb(void *data, Evas_Object *obj EINA_UNUSED,
                                     Elm_Selection_Data *ev)
{
   ...
   if (!panel || !ev || !ev->data || !ev->len)
     return EINA_FALSE;

   text = strndup(ev->data, ev->len);
   if (!text)
     return EINA_FALSE;

   opened = _edi_mainview_panel_drop_open_list(panel, text);
   free(text);
...
{
   if (panel->drop_target_enabled)
     elm_drop_target_del(panel->content, ELM_SEL_FORMAT_URILIST | ELM_SEL_FORMAT_TEXT,
                         NULL, NULL, NULL, NULL, NULL, NULL,
                         _edi_mainview_panel_external_drop_cb, panel);
...
edi_mainview_panel_add(Evas_Object *parent)
{
   ...
   panel->drop_target_enabled = elm_drop_target_add(panel->content,
                                                     ELM_SEL_FORMAT_URILIST | ELM_SEL_FORMAT_TEXT,
                                                     NULL, NULL, NULL, NULL, NULL, NULL,
                                                     _edi_mainview_panel_external_drop_cb, panel);
   return panel;
}
