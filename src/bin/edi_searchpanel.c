...
static void
_edi_searchpanel_placeholder_set(Elm_Code *code)
{
   if (elm_code_file_lines_get(code->file) == 0)
     elm_code_file_line_append(code->file, "", 0, NULL);
}

static void
_edi_searchpanel_placeholder_clear(Elm_Code *code)
{
   Elm_Code_Line *line;
   const char *text;
   unsigned int length = 0;

   if (elm_code_file_lines_get(code->file) != 1)
     return;
...
static void
_edi_searchpanel_follow_tail(Elm_Code *code)
{
   ...
   if (code == _elm_code)
     widget = _info_widget;
   else if (code == _tasks_code)
     widget = _tasks_widget;
...
main_loop_line_append_async(void *data)
{
   ...
   _edi_searchpanel_placeholder_clear(log->logger);
   ...
   _edi_searchpanel_follow_tail(log->logger);
...
edi_searchpanel_find(const char *text)
{
   ...
   _edi_searchpanel_placeholder_set(_elm_code);
   _edi_searchpanel_follow_tail(_elm_code);
...
edi_taskspanel_find(void)
{
   ...
   _edi_searchpanel_placeholder_set(_tasks_code);
   _edi_searchpanel_follow_tail(_tasks_code);
