...
static void
_edi_logpanel_placeholder_set(void)
{
   if (elm_code_file_lines_get(_elm_code->file) == 0)
     elm_code_file_line_append(_elm_code->file, "", 0, NULL);
}

static void
_edi_logpanel_placeholder_clear(void)
{
   Elm_Code_Line *line;
   const char *text;
   unsigned int length = 0;

   if (elm_code_file_lines_get(_elm_code->file) != 1)
     return;

   line = elm_code_file_line_get(_elm_code->file, 1);
   if (!line)
     return;
...
static void
_edi_logpanel_follow_tail(void)
{
   unsigned int lines;

   if (!_info_widget || !_elm_code || !_elm_code->file)
     return;

   lines = elm_code_file_lines_get(_elm_code->file);
   if (lines == 0)
     return;

   elm_code_widget_cursor_position_set(_info_widget, lines, 1);
}
