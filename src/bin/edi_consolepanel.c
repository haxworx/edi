...
static Evas_Object *_edi_console_widget, *_edi_test_widget;
static void _edi_test_line_callback(const char *content);

static void
_edi_code_file_placeholder_set(Elm_Code_File *file)
{
   if (elm_code_file_lines_get(file) == 0)
     elm_code_file_line_append(file, "", 0, NULL);
}

static void
_edi_code_file_placeholder_clear(Elm_Code_File *file)
{
   Elm_Code_Line *line;
   const char *text;
   unsigned int length = 0;

   if (elm_code_file_lines_get(file) != 1)
     return;
...
static void
_edi_code_widget_follow_tail(Evas_Object *widget, Elm_Code *code)
{
   unsigned int lines;

   if (!widget || !code || !code->file)
     return;

   lines = elm_code_file_lines_get(code->file);
   if (lines == 0)
     return;

   elm_code_widget_cursor_position_set(widget, lines, 1);
}
...
static void _edi_consolepanel_append_line_type(const char *line, Eina_Bool err)
{
   ...
   _edi_code_file_placeholder_clear(_edi_console_code->file);

   elm_code_file_line_append(_edi_console_code->file, line, strlen(line),
                             err && _current_dir ? strdup(_current_dir) : NULL);
   _edi_code_widget_follow_tail(_edi_console_widget, _edi_console_code);
...
void edi_consolepanel_clear()
{
   ...
   _edi_code_file_placeholder_set(_edi_console_code->file);
   _edi_code_file_placeholder_set(_edi_test_code->file);
   _edi_code_widget_follow_tail(_edi_console_widget, _edi_console_code);
   _edi_code_widget_follow_tail(_edi_test_widget, _edi_test_code);
...
_edi_test_output_suite(int count, int pass, int fail)
{
   ...
   _edi_code_file_placeholder_clear(_edi_test_code->file);
...
void edi_consolepanel_add(Evas_Object *parent)
{
   ...
   _edi_code_file_placeholder_set(code->file);
