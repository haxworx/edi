...
static Evas_Object *_edi_logpanel, *_edi_consolepanel, *_edi_testpanel, *_edi_searchpanel, *_edi_taskspanel, *_edi_aipanel, *_edi_debugpanel;
static Elm_Object_Item *_edi_logpanel_item, *_edi_consolepanel_item, *_edi_testpanel_item, *_edi_searchpanel_item, *_edi_taskspanel_item, *_edi_aipanel_item, *_edi_debugpanel_item;
static Elm_Object_Item *_edi_selected_bottompanel;
static Evas_Object *_edi_filepanel, *_edi_filepanel_icon;
static const char *_edi_filepanel_selected_path;
...
static Evas_Object *_edi_menu_init, *_edi_menu_commit, *_edi_menu_push, *_edi_menu_pull, *_edi_menu_status, *_edi_menu_stash, *_edi_menu_stash_pop, *_edi_menu_terminate;
...
static void
_edi_bottompane_resize_state_update(void)
{
   double size;
   Eina_Bool collapsed;
   ...
   collapsed = (size <= 0.0001);

   elm_panes_fixed_set(_edi_bottompanes, collapsed);
   elm_object_style_set(_edi_bottompanes, collapsed ? "flush" : "default");
}
...
_edi_slide_panel_free(Elm_Transit_Effect *effect, Elm_Transit *transit EINA_UNUSED)
{
   ...
   if (!slide->left)
     {
        if (!slide->expand && _edi_bottompanes)
          elm_panes_content_right_size_set(_edi_bottompanes, 0.0);
        _edi_bottompane_resize_state_update();
     }
...
_edi_toggle_file_panel(void *data, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
   ...
   if (size == 0.0)
     {
        edi_filepanel_restore_pending_cancel();
        ...
        selected = edi_filepanel_selected_path_get(panel);
        if (_edi_filepanel_selected_path &&
            (!selected || strcmp(selected, _edi_filepanel_selected_path)))
          edi_filepanel_select_path(_edi_filepanel_selected_path);
     }
   else
     {
        edi_filepanel_restore_pending_cancel();
        selected = edi_filepanel_selected_path_get(panel);
        if (selected)
          eina_stringshare_replace(&_edi_filepanel_selected_path, selected);
   ...
   if (item == _edi_selected_bottompanel)
     {
        ...
        elm_panes_fixed_set(_edi_bottompanes, EINA_TRUE);
   ...
   else
     {
        ...
        if (size == 0.0)
          ...
          {
             elm_panes_fixed_set(_edi_bottompanes, EINA_FALSE);
...
edi_aipanel_show()
{
   if (_edi_selected_bottompanel != _edi_aipanel_item)
     elm_toolbar_item_selected_set(_edi_aipanel_item, EINA_TRUE);
}
...
edi_content_setup(Evas_Object *win, const char *path)
{
   ...
   _edi_aipanel_item = elm_toolbar_item_append(tb, edi_theme_icon_path_get("go-up"), _("AI"),
                                               _edi_toggle_panel, "6");
   _edi_toolbar_separator_add(tb);
   ...
   evas_object_size_hint_weight_set(_edi_aipanel, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);
   evas_object_size_hint_align_set(_edi_aipanel, EVAS_HINT_FILL, EVAS_HINT_FILL);
   edi_aipanel_add(_edi_aipanel);
   elm_table_pack(logpanels, _edi_aipanel, 0, 0, 1, 1);
   ...
   if (_edi_project_config->gui.bottomopen)
     {
        ...
        else if (_edi_project_config->gui.bottomtab == 6)
          {
             elm_toolbar_item_icon_set(_edi_aipanel_item, edi_theme_icon_path_get("go-down"));
             _edi_selected_bottompanel = _edi_aipanel_item;
          }
        else
          {
   ...
   if (edi_project_mode_get())
     {
        ...
        elm_object_item_disabled_set(_edi_menu_stash_pop, !can_scm);
     }
}
...
_edi_build_display_status_cb(int status, void *data)
{
   ...
   edi_screens_desktop_notify(_edi_main_win, eina_strbuf_string_get(title), eina_strbuf_string_get(message), status);
...
static void
_edi_menu_select_all_cb(void *data EINA_UNUSED, Evas_Object *obj EINA_UNUSED,
                        void *event_info EINA_UNUSED)
{
...
static void
_edi_menu_scm_stash_pop_cb(void *data EINA_UNUSED, Evas_Object *obj EINA_UNUSED,
                           void *event_info EINA_UNUSED)
{
   edi_consolepanel_show();
   edi_scm_stash_pop();
...
_edi_menu_setup(Evas_Object *win)
{
   ...
   elm_menu_item_add(menu, menu_it, edi_theme_icon_path_get("edit-select-all"), _("Select All"), _edi_menu_select_all_cb, NULL);
   elm_menu_item_separator_add(menu, menu_it);
   ...
   if (edi_project_mode_get())
     {
        ...
        _edi_menu_build = elm_menu_item_add(menu, menu_it, edi_theme_icon_path_get("applications-development"), _("Build"), _edi_menu_build_cb, NULL);
        ...
        elm_menu_item_add(menu, menu_it, edi_theme_icon_path_get("dialog-error"), _("Memcheck"), _edi_menu_memcheck_cb, NULL);
        ...
        _edi_menu_stash_pop = elm_menu_item_add(menu, menu_it, edi_theme_icon_path_get("edit-redo"), _("Stash Pop"), _edi_menu_scm_stash_pop_cb, NULL);
        elm_menu_item_separator_add(menu, menu_it);
...
edi_toolbar_setup(void)
{
   ...
   if (edi_project_mode_get())
     {
        _edi_toolbar_build = _edi_toolbar_item_add(tb, "applications-development", _("Build"), _tb_build_cb);
...
elm_main(int argc EINA_UNUSED, char **argv EINA_UNUSED)
{
   ...
   if (!project_path)
   ...
   else if (!ecore_file_is_dir(project_path))
     {
        ...
        if ((mime) && (!edi_content_provider_for_mime_get(mime)))
          {
   ...
   eina_stringshare_del(_edi_filepanel_selected_path);
   _edi_filepanel_selected_path = NULL;
   _edi_log_shutdown();
   edi_listen_shutdown();
   elm_shutdown();
   edi_scm_shutdown();
