...
_edi_project_config_cb_free(void)
{
   ...
   if (_edi_project_config->debug_command) eina_stringshare_del(_edi_project_config->debug_command);
   if (_edi_project_config->user_fullname) eina_stringshare_del(_edi_project_config->user_fullname);
   if (_edi_project_config->user_email) eina_stringshare_del(_edi_project_config->user_email);
   if (_edi_project_config->agent.provider) eina_stringshare_del(_edi_project_config->agent.provider);
   if (_edi_project_config->agent.model) eina_stringshare_del(_edi_project_config->agent.model);
   if (_edi_project_config->agent.endpoint) eina_stringshare_del(_edi_project_config->agent.endpoint);
   if (_edi_project_config->agent.api_key) eina_stringshare_del(_edi_project_config->agent.api_key);
   if (_edi_project_config->agent.project_id) eina_stringshare_del(_edi_project_config->agent.project_id);
   ...
   if (_edi_project_config)
     {
        ...
        else if ((_edi_project_config->version & 0xffff) < EDI_PROJECT_CONFIG_FILE_GENERATION)
          save = EINA_TRUE;
     }
...
#define IFPCFGEND }
   ...
   IFPCFG(0x0006);
   /* Reserved migration step: internal icon toggle removed, always use theme icons. */
   IFPCFGEND;

   IFPCFG(0x0008);
   _edi_project_config->agent.enabled = EINA_FALSE;
   _edi_project_config->agent.provider = eina_stringshare_add("google_codex");
   _edi_project_config->agent.model = eina_stringshare_add("gemini-2.5-flash");
   _edi_project_config->agent.endpoint = eina_stringshare_add("");
   _edi_project_config->agent.api_key = eina_stringshare_add("");
   _edi_project_config->agent.project_id = eina_stringshare_add("");
