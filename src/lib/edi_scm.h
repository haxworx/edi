...
/**
 * Get diff output for a specific commit hash.
 *
 * @param commit The commit hash to inspect.
 *
 * @return diff output as a string, or NULL if unavailable/invalid.
*/
EAPI char *edi_scm_commit_diff(const char *commit);
...
/**
 * Get diff of changes for a single file in repository.
 *
 ...
 * @return diff output as a string.
*/
EAPI char *edi_scm_diff_path(const char *path, Eina_Bool cached);
