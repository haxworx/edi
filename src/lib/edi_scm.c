...
static Eina_Bool
_edi_scm_commit_hash_valid(const char *commit)
{
   size_t len;
   ...
   for (size_t i = 0; i < len; i++)
     {
        if (!isxdigit((unsigned char)commit[i]))
          return EINA_FALSE;
     }
...
EAPI char *
edi_scm_commit_diff(const char *commit)
{
   Eina_Strbuf *command;
   char *diff;

   if (!_edi_scm_commit_hash_valid(commit))
     return NULL;

   command = eina_strbuf_new();
   eina_strbuf_append_printf(command, "git show --no-color %s", commit);
   diff = _edi_scm_exec_response(eina_strbuf_string_get(command));
   eina_strbuf_free(command);
...
EAPI char *
edi_scm_diff_path(const char *path, Eina_Bool cached)
{
   Eina_Strbuf *command;
   char *output;
   char *escaped;
   ...
   if (cached)
     eina_strbuf_append_printf(command, "git diff --cached -- %s", escaped);
   else
     eina_strbuf_append_printf(command, "git diff -- %s", escaped);
...
edi_scm_avatar_url_get(const char *email)
{
   ...
   const char *url;
   SHA256_CTX ctx;
   char shaout[(2 * SHA256_BLOCK_SIZE) + 1];
   ...
   id = strndup(start, end - start);
   eina_str_tolower(&id);

   sha256_init(&ctx);
   sha256_update(&ctx, (unsigned char const *)id, strlen(id));
   sha256_final(&ctx, hash);

   for (n = 0; n < SHA256_BLOCK_SIZE; n++)
     {
        shaout[2 * n] = hex[hash[n] >> 4];
        shaout[2 * n + 1] = hex[hash[n] & 0x0f];
     }
   shaout[2 * SHA256_BLOCK_SIZE] = '\0';

   url = eina_stringshare_printf("https://gravatar.com/avatar/%s", shaout);
